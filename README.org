...
with open("records.mrc", "rb") as f:
    ...
    # Get all 008 and 264$c values
    all_008 = reader.get_all_values("008")
    all_264c = reader.get_all_values("264$c")
    ...
    for i in range(len(reader)):
        year = None
        # Try 008 first
        if all_008[i] and len(all_008[i][0]) >= 11:
            year = all_008[i][0][7:11]
            if year.isdigit():
                source['008'] += 1
        # Fall back to 264$c
        elif all_264c[i]:
            match = re.search(r'(19|20)\d{2}', all_264c[i][0])
            if match:
                year = match.group(0)
                source['264$c'] += 1
